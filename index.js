                ...
                fs.readFile('data/' + file, (err, data) => {
                    ...
                    if (err)
                        ...
                        for (let i = 0; i < data.length; i++) {
                            ...
                            let history = [];
                            let lat, long, setting = '', elevation, stability = '', description = '', marker = '', magnetic = '';
                            ...
                            for (const line of data[i]) {
                                ...
                                if (descStarted)
                                    ...
                                    if (line.includes('*') && line.includes('POSITION')) {
                                        ...
                                        history.push({ date: split[0], condition: split.slice(1, split.length - 1).join(' '), reporter: split[split.length - 1] });
                                    }
                                ...
                                markers.push({ description, elevation, history: history.slice(1), id, lat, long, magnetic, marker, setting, stability });
                            } catch (e) { console.error('Parsing failed for ' + file + '[' + i + ']', e) }
                        }
                        const limit = Math.ceil(JSON.stringify(markers, null, "\t").length / 20 / 1024 / 1024);
                        for (let i = 0; i < limit; i++)
                            fs.writeFile('output/' + file.substring(0, 2) + (limit > 1 ? i : '') + '.json', JSON.stringify({ date, region, markers: markers.slice(i * markers.length / limit, (i + 1) * markers.length / limit) }, null, "\t"), (err) => { if (err) throw err; });
                        filesDone++;
                        // console.clear();
